...
- **C 계산에서의 오프바이원 버그**: 윈도우가 `s -> s+1`로 한 칸 이동하면 실제로는 "`s` 위치의 값이 빠지고 `s + win_len` 위치의 값이 새로 들어오는" 것인데, 처음에는 새로 들어오는 값을 현재 오른쪽 끝 `e`가 속한 **덩어리**의 값으로 착각했습니다. `e`가 덩어리 경계의 마지막 칸일 경우 실제로 새로 들어오는 값은 다음 덩어리 것이라 diff가 완전히 틀어졌고, 그 결과 경계 부근에서 C가 실제보다 작게(심하면 0으로) 나오는 오답이 반복됐습니다.
- **수정**: diff를 결정하는 기준을 "`e`가 속한 덩어리"가 아니라 "`s + win_len`(다음에 새로 들어올 위치)이 속한 덩어리"로 바꾸고, step 한계도 그 위치 기준으로 다시 계산했습니다. `steps = min(end_idx[i] - s + 1, end_idx[j] - (s + win_len) + 1, max_s - s + 1)` 만큼 한꺼번에 점프하여 $O(N)$에 $C$를 정확히 카운트합니다.
