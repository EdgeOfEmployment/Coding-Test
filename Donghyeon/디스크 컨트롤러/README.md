...
최단 작업 우선(SJF, Shortest Job First) 스케줄링을 시뮬레이션으로 구현함.

1. `jobs`를 요청 시각(`s`) 기준으로 오름차순 정렬.
2. 현재 시각(`currentTime`)까지 요청이 들어온 작업들을 모두 대기 큐(`waitingQueue`)에 넣음.
3. 대기 큐가 비어있지 않다면 소요 시간(`l`)이 가장 짧은 작업을 골라 처리하고, 현재 시각을 그만큼 증가시키며 반환 시간(완료 시각 − 요청 시각)을 누적.
4. 대기 큐가 비어있다면(하드디스크가 쉬는 상태) 다음 작업이 요청되는 시각으로 현재 시각을 바로 점프시켜 불필요한 반복을 줄임.
5. 모든 작업이 끝나면 누적한 반환 시간의 합을 작업 개수로 나눠 정수부만 반환.

매 시점 우선순위 큐 전체를 정렬해도 `jobs`의 길이가 최대 500이라 시간 제한 안에서 충분히 통과하며, 대기 큐가 빌 때 시간을 건너뛰는 처리가 없으면 무한루프에 빠질 수 있다는 점이 이 문제의 함정이어서 이부분만 확인
