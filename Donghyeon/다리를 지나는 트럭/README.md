...
큐(Queue)를 이용해 다리 위 상태를 그대로 시뮬레이션

1. `bridge_length` 길이만큼 0으로 채운 큐를 다리로 삼고, 매 초(1틱)마다 큐의 맨 앞 값을 다리에서 빠져나가는 트럭 무게로 간주해 `sum`(다리 위 총 무게)에서 뺌.
2. 대기 중인 다음 트럭의 무게를 더했을 때 `weight`를 넘지 않으면 그 트럭을 큐에 push하고 대기열 인덱스를 다음으로 넘기며, 넘는다면 자리만 채우는 의미로 0을 push(다리에 못 올라간 트럭은 대기).
3. 모든 트럭이 다리에 진입할 때까지 이 과정을 반복하며 `answer`(경과 시간)를 1씩 증가.
4. 마지막 트럭이 다리에 올라간 시점에 반복이 끝나므로, 그 트럭이 다리를 완전히 건너는 데 걸리는 `bridge_length`를 더해 반환.

큐의 길이를 다리 길이로 고정해두면 "몇 번째 칸에 어떤 트럭이 있는지"를 인덱스 계산 없이 자연스럽게 표현가능
