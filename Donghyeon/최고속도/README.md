...
문제를 정리하면 "1번 도시에서 각 도시로 이동할 때, 경로 위에서 만나는 카메라 제한 속도 중 최솟값을 최대로 만드는 경로를 찾아라"는 문제
단순 최단 경로가 아니라 **경로 상 가장 낮은 제한(병목)을 최대화하는 문제**, 즉 최대 병목 경로(widest path) 문제

### 그래프로 압축하기

좌표값이 최대 10^9까지 가지만, 실제로 의미 있는 지점은 다음 네 종류뿐

- 각 도로의 양 끝점
- 각 도로 정중앙의 카메라 지점
- 서로 다른 도로끼리의 교차점 (서로 다른 두 도로는 최대 한 점에서만 만난다는 제약 덕분에 계산이 단순함)
- 도시의 위치

이 지점들만 노드로 잡고, 같은 도로 위에서 좌표 순으로 인접한 두 지점끼리만 간선으로 이으면, 좌표 범위와 무관하게 도로/도시 개수(최대 수천 개 수준)에 비례하는 크기의 그래프로 압축할 수 있다. 한 지점에 카메라가 여러 개 겹치는 경우(다른 도로의 카메라나 교차점과 같은 좌표)에는 해당 노드의 `limit`을 만날 때마다 `Math.min`으로 갱신해서, "가장 낮은 제한을 따른다"는 조건을 자연스럽게 처리함

### 변형된 다익스트라 (최대 병목 경로)

이 문제는 간선이 아니라 **노드(카메라)에 제한이 걸려 있는** 구조라서, 각 노드에 `limit`(카메라가 없으면 `Infinity`)을 부여하고 `dist[v]`를 "출발 도시에서 v까지 갈 때 가능한 최고 속도(경로 위 최소 limit의 최댓값)"로 정의

- 시작 노드(1번 도시)의 `dist`는 `Infinity`로 두고, 최소 힙 대신 **최대 힙**으로 "지금까지 확정된 값 중 가장 큰 값"을 먼저 꺼내며 확장. 일반 다익스트라가 min-heap + 덧셈으로 최단 거리를 구하는 것과 반대로, 여기서는 max-heap + `Math.min`으로 병목 값을 최대화하는 방향으로 뒤집은 것뿐
- 인접 노드로 넘어갈 때 `nextLimit = Math.min(현재까지의 병목, 다음 노드의 limit)`로 계산하고, 기존에 기록된 값보다 더 커질 때만(더 여유 있는 경로를 찾았을 때만) 갱신하고 큐에 다시 넣는다.
- 간선 가중치가 없고 완화 방향이 "더 큰 값으로만 갱신"이라는 점만 다를 뿐, 한 번 확정된 노드는 이후 더 나은 값으로 갱신될 수 없다는 다익스트라의 핵심 성질은 그대로 성립

### 결과 계산

각 도시 노드의 `dist` 값이 `Infinity`라면 출발지부터 그 도시까지 카메라를 한 번도 거치지 않았다는 뜻이므로 문제 조건대로 0을 담고, 그렇지 않으면 `dist` 값을 그대로 결과 배열에 담아 반환함
