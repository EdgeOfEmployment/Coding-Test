...
1. `plans`를 시작 시각(분 단위) 기준으로 오름차순 정렬
2. 앞에서부터 순회하며, 현재 과제의 시작 시각부터 다음 과제의 시작 시각까지 남는 시간(`availableTime`)을 계산
    - 현재 과제의 소요 시간이 `availableTime` 이내라면 과제를 완료 처리하고, 남는 시간만큼 스택에 쌓여있는(잠시 멈춘) 과제들을 최근 순(LIFO)으로 꺼내 이어서 진행, 다 끝내지 못하면 남은 시간을 갱신해 다시 스택에 넣음.
    - 소요 시간이 `availableTime`보다 크면 과제를 끝내지 못한 것이므로, 남은 작업 시간을 계산해 스택에 push하고 다음 과제로.
3. 마지막 과제는 다음 과제가 없으므로 루프 밖에서 바로 완료 처리.
4. 순회가 끝난 뒤 스택에 남아있는 과제들은 "가장 최근에 멈춘 과제부터" 처리해야 하므로 pop 순서 그대로 결과에 추가.

"잠시 멈춘 과제 중 가장 최근 것부터 재개한다"는 조건이 스택(LIFO) 구조와 정확히 대응된다는 점이 이 문제의 핵심
