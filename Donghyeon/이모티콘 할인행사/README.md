...
- 완전탐색(브루트포스): 이모티콘 개수 `m ≤ 7`, 할인율 후보는 10/20/30/40%의 4가지뿐이므로 가능한 조합은 최대 `4^7 = 16,384`가지
- `itertools.product(discount_rates, repeat=len(emoticons))`로 모든 이모티콘의 할인율 조합을 생성
- 각 조합마다 모든 사용자(`n ≤ 100`)를 순회하며 실제 지불 금액(`user_spend`)을 계산
  - 사용자가 요구하는 할인율 이상인 이모티콘만 구매 대상에 포함
  - `user_spend`가 사용자의 예산(가격) 이상이면 구매를 모두 취소하고 이모티콘 플러스 가입자로 카운트
  - 그렇지 않으면 해당 금액을 판매액에 누적
- 목표 우선순위를 그대로 비교 로직에 반영: 가입자 수(`current_plus`)가 더 크면 갱신, 같다면 판매액(`current_revenue`)이 더 큰 경우에만 갱신

### 시간 복잡도

- 조합 수 `4^m`(최대 16,384) × 사용자 수 `n`(최대 100) × 이모티콘 수 `m`(최대 7) ≈ 최대 약 1,100만 번 연산
- `m ≤ 7`이라는 작은 제약이 완전탐색을 허용하는 핵심 힌트

### 느낀 점

- 제약 조건의 크기(이모티콘 개수 ≤ 7)를 보고 완전탐색이 통할지 먼저 가늠하는 습관이 중요함을 다시 확인함
- "가입자 수 최대화가 1순위, 판매액 최대화가 2순위"라는 다중 목표 최적화를 `if`/`elif` 비교만으로 간단히 구현할 수 있었음
